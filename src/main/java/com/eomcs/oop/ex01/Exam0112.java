...
public class Exam0112 {
  public static void main(String[] args) {
    ...
    // => 밥, 국, 반찬1, 반찬2, 반찬3 을 쟁반에 담아서 가져오는 상황. (식판 생각)
    ...
    //   => s 객체의 필드 값을 설정한다.
    //
    // 레퍼런스 s가 가리키는 인스턴스의 name 필드 값
    // 
    // 객체라는 말은 인스턴스를 가리키는 말이 아니라 클래스, 인스턴스 등등 
    // 문맥에 따라 알아들어야 한다.
    s.name = "홍길동"; // s에 저장된 인스턴스 주소로 찾아가서 name 항목에 값 저장
    s.kor = 100; // s에 저장된 인스턴스 주소로 찾아가서 kor 항목에 값 저장
    s.eng = 90;
    s.math = 80;
...
//     Score s;  // 아직 Score 설계도에 따라 변수들이 준비된 상태가 아니다. //주소를 담을 메모리만 준비
// - 클래스의 설계도에 따라 메모리를 준비하려면 따로 new 명령을 사용해야 한다.
//     new Score(); -> 인스턴스 주소 준비 완료!
// - 생성된 메모리를 사용하려면 주소를 잘 보관해 두어야 한다.
//     s = new Score(); -> Score의 설계도에 따라 생성된 인스턴스 주소를 s 레퍼런스에 보관
