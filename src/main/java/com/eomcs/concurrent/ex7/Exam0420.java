...
public class Exam0420 {
  ...
  static class MyRunnable implements Runnable {
    ...
    public void run() {
      try {
        ...
        //        Thread.sleep(millisec); // Not Runnable 상태 -> 스레드 종료기회
        for (long i = 0; i < 10000000; i++) {
          double r = Math.tan(3456.78999) / Math.random();
        }
  ...
  public static void main(String[] args) {
    ...
    //      하지만 실행 중인 작업의 경우에는 취소 될 수도 안될수도
    // => 작업 중인 스레드가 Not Runnable 상태가 아니라면 작업이 끝날 때까지 기다린다. 
    // => 다만 작업을 마치고 대기 중인 스레드는 즉시 취소한다. // 대기 중인 스레드의 경우에는 100% 취소
