...
public class Receiver {
  ...
  public static void main(String[] args) throws Exception {
    ...
    // - 포트번호
    //   - 1024 ~ 49151 사이의 값을 사용한다.
    //   - 같은 컴퓨터에서 이미 사용중인 포트 번호는 지정할 수 없다.
    //   - 예) Oracle DMBS(1521), MySQL DMBS(3306) 등
    ...
    // OutputStream을 리턴한다는 것 -> OutputStream을 상속받은 서브 클래스의 객체를 리턴한다는 뜻
    // PrintStream() 을 데코레이터로 사용
    Scanner in = new Scanner(socket.getInputStream());
    // Scanner() 를 데코레이터로 사용 (실제 데코레이터는 아니다 - 같은 조상을 가지고 있지 않음)
